...
export const closeLoanPosition: FlowDeclaration<CloseLoanPositionRequest> = {
  ...
  Details({ request }) {
    ...
    const amountToRepay = collPrice.data !== undefined ? (
      repayWithCollateral
        ? (dn.div(loan.borrowed ?? dn.from(0), collPrice.data))
        : (loan.borrowed ?? dn.from(0))
      ) : undefined;

    const collToReclaim = amountToRepay !== undefined ? (
      repayWithCollateral
    ...
    if (!collPrice.data || !amountToRepay || !collToReclaim) {
      return null;
    }
  ...
  async getSteps(ctx) {
    ...
    const { entireDebt: entireDebtBN } = await ctx.readContract({
      ...coll.contracts.TroveManager,
      functionName: "getLatestTroveData",
      args: [BigInt(loan.troveId)],
    });
