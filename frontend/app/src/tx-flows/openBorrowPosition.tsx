...
export const openBorrowPosition: FlowDeclaration<OpenBorrowPositionRequest> = {
        ...
        <TransactionDetailsRow
          ...
          value={[
            ...
            <Amount
              ...
              fallback="…"
              prefix="$"
              value={collPrice.data && displayedCollAmount && dn.mul(displayedCollAmount, collPrice.data)}
            />,
          ]}
        />
        <TransactionDetailsRow
  ...
  steps: {
    ...
    approveLst: {
      ...
      async commit(ctx) {
        ...
        const approvalAmount = ctx.preferredApproveMethod === "approve-infinite" ?
          maxUint256 :
          await getApprovalAmount(branch.symbol, dn.abs(ctx.request.collAmount[0]), ctx);
    ...
    openTroveLst: {
      ...
      async commit(ctx) {
        ...
        const normalizedCollAmount = await getStERC20Amount(branch.symbol, ctx.request.collAmount, ctx);
  ...
  async getSteps(ctx) {
    ...
    if (allowance < normalizedCollAmount[0]) {
