...
export const updateBorrowPosition: FlowDeclaration<UpdateBorrowPositionRequest> = {
  ...
  Details({ request }) {
    ...
    const displayedCollChange = useStERC20Amount(loan.branchId, collChange);
    const collateral = getCollToken(loan.branchId);
  ...
  steps: {
    ...
    approveColl: {
      ...
      async commit(ctx) {
        ...
        const approvalAddress = getApprovalAddress(branch.symbol);
        const approvalAmount = ctx.preferredApproveMethod === "approve-infinite" ?
          maxUint256 :
          await getApprovalAmount(branch.symbol, dn.abs(collChange), ctx);
    ...
    depositColl: {
      ...
      async commit(ctx) {
        ...
        const branch = getBranch(loan.branchId);
        const normalizedCollChange = await getStERC20Amount(branch.symbol, dn.abs(collChange), ctx);
  ...
  async getSteps(ctx) {
    ...
    const isCollApproved = await (async () => {
      ...
      const normalizedCollChange = await getStERC20Amount(branch.symbol, collChange, ctx);
